...
const FeatureCard = ({ icon, title, description, index }: FeatureCardProps) => {
  ...
  return (
    <div 
      ref={cardRef}
      className={cn(
        "feature-card glass-card opacity-1 p-4 sm:p-6",
        "lg:hover:bg-gradient-to-br lg:hover:from-white lg:hover:to-[hsl(var(--primary)/0.04)]",
        "transition-all duration-300"
      )}
      style={{ animationDelay: `${0.1 * index}s` }}
    >
      <div className="rounded-full bg-[hsl(var(--primary)/0.08)] w-10 h-10 sm:w-12 sm:h-12 flex items-center justify-center text-[hsl(var(--primary))] mb-4 sm:mb-5">
        {icon}
      </div>
      <h3 className="text-lg sm:text-xl font-semibold mb-2 sm:mb-3">{title}</h3>
      <p className="text-[hsl(var(--muted-foreground))] text-sm sm:text-base">{description}</p>
    </div>
  );
};
      ...
      <div className="section-container">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 md:gap-8">
          ...
          <FeatureCard
            ...
            title="Nhận thức không gian"
            description="Cảm biến và công nghệ bản đồ tiên tiến giúp Atlas di chuyển dễ dàng trong môi trường phức tạp."
            index={3}
          />
          <FeatureCard
            ...
            description="Các giao thức tích hợp bảo vệ dữ liệu và quyền riêng tư của bạn, cùng các biện pháp an toàn vật lý."
            index={4}
          />
          <FeatureCard
            ...
            title="Hỗ trợ công việc"
            description="Từ nhắc nhở đơn giản đến các tác vụ phức tạp nhiều bước, Atlas có thể hỗ trợ nhiều hoạt động khác nhau."
            index={5}
          />
        </div>
      </div>
